...
malignant = []
benign = []
for name in glob.glob("Descriptions/ISIC*"):
    try:
...
descriptions_location = "Descriptions/"
...
try:
    os.makedirs("train/meta")
    os.makedirs("train/img")
except:
    print("Train directories already exist")
for file in train_files:
    try:
        shutil.move(descriptions_location + file, "train/meta/" + file)
        shutil.move(images_location + file + ".jpeg", "train/img/" + file + ".jpeg")
    except:
...
try:
    os.makedirs("test/meta")
    os.makedirs("test/img")
except:
    print("Test directories already exist")
for file in test_files:
    try:
        shutil.move(descriptions_location + file, "test/meta/" + file)
        shutil.move(images_location + file + ".jpeg", "test/img/" + file + ".jpeg")
    except:
...
# train
train_meta = glob.glob("train/meta/ISIC*")
train_meta = [x[11:23] for x in train_meta]
print(len(train_meta))
train_img = glob.glob("train/img/ISIC*")
...
for name in train_meta:
    if name not in train_both:
        try:
            os.remove("train/meta/" + name)
        except:
            print("Error removing {}".format(name))
for name in train_img:
    if name not in train_both:
        try:
            os.remove("train/img/" + name + "jpeg")
        except:
            print("Error removing {}".format(name))

# test
test_meta = glob.glob("test/meta/ISIC*")
test_meta = [x[10:22] for x in test_meta]
print(len(test_meta))
test_img = glob.glob("test/img/ISIC*")
...
for name in test_meta:
    if name not in test_both:
        try:
            os.remove("test/meta/" + name)
        except:
            print("Error removing {}".format(name))
for name in test_img:
    if name not in test_both:
        try:
            os.remove("test/img/" + name + "jpeg")
        except:
            print("Error removing {}".format(name))
